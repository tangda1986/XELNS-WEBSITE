  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        insertAtCursor(`<img src="${compressedBase64}" class="w-full rounded-lg shadow-md my-6" alt="Uploaded Image" />`);
      } catch (error) {
    ...
    <div className="space-y-2">
      ...
      <div className="border rounded-lg overflow-hidden bg-white shadow-sm focus-within:ring-2 focus-within:ring-brand-500 transition-shadow">
        ...
        <div className="bg-gray-50 border-b p-2 flex gap-1 flex-wrap items-center select-none">
           <ToolButton icon={Bold} onClick={() => insertTag('<b>', '</b>')} tooltip="加粗" />
           <ToolButton icon={Italic} onClick={() => insertTag('<i>', '</i>')} tooltip="斜体" />
           <div className="w-px h-5 bg-gray-300 mx-1"></div>
           
           <ToolButton icon={Heading} onClick={() => insertTag('<h3 class="text-xl font-bold mb-3 mt-6 text-gray-900">', '</h3>')} tooltip="标题 (H3)" />
           <ToolButton icon={AlignLeft} onClick={() => insertTag('<p class="mb-4 leading-relaxed text-gray-700">', '</p>')} tooltip="段落 (P)" />
           <div className="w-px h-5 bg-gray-300 mx-1"></div>
           
           <ToolButton icon={List} onClick={() => insertTag('<ul class="list-disc list-inside space-y-2 mb-4 text-gray-700">', '</ul>')} tooltip="无序列表" />
           <ToolButton icon={Check} onClick={() => insertTag('<li>', '</li>')} tooltip="列表项" />
           <div className="w-px h-5 bg-gray-300 mx-1"></div>
           ...
           <ToolButton icon={Image} onClick={() => {
              ...
              if(url) insertAtCursor(`<img src="${url}" class="w-full rounded-lg shadow-md my-6" alt="Image" />`);
           }} tooltip="插入网络图片 (URL)" />
           ...
           <div className="w-px h-5 bg-gray-300 mx-1"></div>
           <ToolButton icon={Code} onClick={() => insertTag('<div class="bg-gray-100 p-4 rounded border">', '</div>')} tooltip="灰色背景框" />
        </div>
      ...
      <div className="flex justify-between items-center text-xs text-gray-500 px-1">
        ...
        <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-600">HTML 源码模式</span>
      </div>
    </div>
  );
};
